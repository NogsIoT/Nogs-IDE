...
//
//---------------------------------------------------------------------------
void __fastcall TFormMemoryTracker::Area(TCanvas *Canvas, uint32_t Address, uint32_t Size, TColor Color)
{
  Address -= BASE;
...
//---------------------------------------------------------------------------
void __fastcall TFormMemoryTracker::DoAllocate(uint32_t Count, uint32_t Address, uint32_t Size)
{
...
//---------------------------------------------------------------------------
void __fastcall TFormMemoryTracker::DoFree(uint32_t Count, uint32_t Address, uint32_t Size)
{
  if (Address) {
...
//
//---------------------------------------------------------------------------
__fastcall TFormMemoryTracker::TFormMemoryTracker(TComponent* Owner)
  : TForm(Owner)
{
  Memory.Length = 128 * 1024;
  //for (int I = 0; I < Memory.Length; I++) Memory[I] = 0xFFFFFFFF;
  Tracker.Length = 100000;
  TrackerP = 0;
  for (int I = 0; I < Tracker.Length;I++) {
    TBuffer *Buffer = &Tracker[I];
    memset(Buffer, 0, sizeof(TBuffer));
  }
}
//---------------------------------------------------------------------------
void __fastcall TFormMemoryTracker::Add(TBuffer *Buffer)
{
  switch (Buffer->Type) {
...
//---------------------------------------------------------------------------
void __fastcall TFormMemoryTracker::PaintBoxPaint(TObject *Sender)
{
...
void __fastcall TFormMemoryTracker::GridDrawCell(TObject *Sender, int Col, int Row, TRect &Rect, TGridDrawState State)
{
  String S;
...
//---------------------------------------------------------------------------
void __fastcall TFormMemoryTracker::FormCloseQuery(TObject *Sender, bool &CanClose)
{
  FormMain->FormMemoryTracker = 0;
}
//---------------------------------------------------------------------------
